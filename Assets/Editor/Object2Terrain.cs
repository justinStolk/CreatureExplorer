...
public class Object2Terrain : EditorWindow {
	...
	void CreateTerrain(){	
        ...
        {
			if (g.TryGetComponent(out MeshCollider col))
				colliders.Add(col);
            else
			{
				//Add a collider to our source object if it does not exist.
				//Otherwise raycasting doesn't work.
				colliders.Add(g.AddComponent<MeshCollider>());
				cleanUp = () => DestroyImmediate(g.GetComponent<MeshCollider>());
			}
        }
		...
		*/
		...
		Bounds bounds = new Bounds(Vector3.zero, Vector3.one*resolution);	
		float sizeFactor = addTerrain.y;
		terrain.size = new Vector3(resolution, 0, resolution) + addTerrain;
		bounds.size = new Vector3(terrain.size.x, addTerrain.y, terrain.size.z);
		...
		foreach (MeshCollider collider in colliders)
		{
			...
			for (int zCount = 0; zCount < maxHeight; zCount++)
			{
				...
				for (int xCount = 0; xCount < maxLength; xCount++)
				{
					...
					if (heights[zCount, xCount] < height)
