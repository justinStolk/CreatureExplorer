...
namespace DevDunk.AutoLOD
{
    ...
    public class AnimatorLODObject : MonoBehaviour
    {
        ...
        void Awake()
        {
            ...
            if (SkinnedMeshRenderers.Length == 0) SkinnedMeshRenderers = GetComponentsInChildren<SkinnedMeshRenderer>();
        ...
        {
            if(!currentState) TrackedAnimatorComponent.enabled = currentState = true;
            if(currentSpeed != speed) TrackedAnimatorComponent.speed = currentSpeed = speed;
        ...
        {
            if(currentState) TrackedAnimatorComponent.enabled = currentState = false;
            FrameCountdown--;
        }
