    ...
    public class AnimatorLODManager : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            for (int i = 0; i < Animators.Count; i++)
            {
                ...
                {
                    var frameCount = GetClosestLODFrameCountBinaray(Animators[i].TrackedTransform.position, camPos, out SkinQuality quality);
                    Animators[i].EnableAnimator(frameCount, frameCount + 1, quality);
                }
                else
                {
